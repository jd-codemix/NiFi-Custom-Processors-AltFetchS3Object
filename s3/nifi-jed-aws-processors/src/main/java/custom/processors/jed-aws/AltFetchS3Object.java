 ...
 */
package custom.processors.jed.aws;
...
import static com.amazonaws.services.s3.S3ResourceType.BUCKET;

@SupportsBatching
@InputRequirement(Requirement.INPUT_REQUIRED)
@Tags({"Amazon", "S3", "AWS", "Get", "Fetch"})
@CapabilityDescription("Checks the existence of an S3 file with options to: retrieve or not retrieve file contents, invoke or not invoke a bulletin (if file does not exist) and penalize or not penalize the flow (if file does not exist). These three new Boolean properties are Include-content, Bulletin, and Penalize. Five new S3 attributes are set during runtime; s3.key (key name), s3.exist (boolean value of key existence), s3.message (success message or failure reason), s3.messagetype (exception type if failure), and s3.statuscode (HTTP status code if failure). The original existing attribute s3.bucket is still available.")
@WritesAttributes({
    @WritesAttribute(attribute = "s3.bucket", description = "The name of the S3 bucket"),
    @WritesAttribute(attribute = "s3.key", description = "The name of the S3 key"),
    @WritesAttribute(attribute = "s3.exist", description = "Boolean value of S3 key existance"),
    @WritesAttribute(attribute = "s3.message", description = "Exception reason"),
    @WritesAttribute(attribute = "s3.messagetype", description = "Exception type"),
    @WritesAttribute(attribute = "s3.statuscode", description = "HTTP status code"),
...
public class AltFetchS3Object extends AbstractS3Processor {
    ...
    public static final PropertyDescriptor BULLETIN = new PropertyDescriptor.Builder()
            .name("bulletin")
            .required(true)
            .description("If true, indicates the error bulletin will involke if S3 file is not found.")
            .addValidator(StandardValidators.BOOLEAN_VALIDATOR)
            .allowableValues(new AllowableValue("true", "True", "Indicates the error bulletin will involke if S3 file is not found."), new AllowableValue("false", "False", "Indicates the error bulletin will not involke if S3 is file not found."))
            .defaultValue("false")
            .build();

    public static final PropertyDescriptor PENALIZE = new PropertyDescriptor.Builder()
            .name("penalize")
            .required(true)
            .description("If true, indicates penalize will involke if S3 file is not found.")
            .addValidator(StandardValidators.BOOLEAN_VALIDATOR)
            .allowableValues(new AllowableValue("true", "True", "Indicates penalize will involke if S3 file is not found."), new AllowableValue("false", "False", "Indicates penalize will not involke if S3 is file not found."))
            .defaultValue("false")
            .build();
    ...
    public static final PropertyDescriptor INCLUDE_CONTENT = new PropertyDescriptor.Builder()
            ...
            .required(true)
            .description("If true, includes file content.")
            .addValidator(StandardValidators.BOOLEAN_VALIDATOR)
            .allowableValues(new AllowableValue("true", "True", "Include file content."), new AllowableValue("false", "False", "Do not include file content."))
            .defaultValue("true")
            .build();

            
    public static final List<PropertyDescriptor> properties = Collections.unmodifiableList(
            Arrays.asList(BUCKET, KEY, REGION, BULLETIN, PENALIZE, INCLUDE_CONTENT, ACCESS_KEY, SECRET_KEY, CREDENTIALS_FILE, AWS_CREDENTIALS_PROVIDER_SERVICE,
                TIMEOUT, VERSION_ID, SSL_CONTEXT_SERVICE, ENDPOINT_OVERRIDE, SIGNER_OVERRIDE, ENCRYPTION_SERVICE, PROXY_CONFIGURATION_SERVICE,
                PROXY_HOST,PROXY_HOST_PORT, PROXY_USERNAME, PROXY_PASSWORD, REQUESTER_PAYS, RANGE_START, RANGE_LENGTH));
    ...
    public void onTrigger(final ProcessContext context, final ProcessSession session) {
        ...
        final boolean bulletin = context.getProperty(BULLETIN).asBoolean();
        final boolean penalize = context.getProperty(PENALIZE).asBoolean();
        final boolean includeContent = context.getProperty(INCLUDE_CONTENT).asBoolean();
        ...
        try (final S3Object s3Object = client.getObject(request)) {
            ...
            attributes.put("s3.key", s3Object.getKey());
            ...
            if (metadata.getContentDisposition() != null) {
                ...
                if (contentDisposition.startsWith("attachment; filename=")) {
                    ...
                    attributes.put("s3.pathCameFrom", "key");
                } else {
                    setFilePathAttributes(attributes, contentDisposition);
                    attributes.put("s3.pathCameFrom", "contentDisposition");
                }
            }
            ...
            attributes.put("s3.exist", "false");
            attributes.put("s3.messagetype", "IOException");
            attributes.put("s3.message", ioe.getMessage());
            ...
            attributes.put("s3.key", key);
            attributes.put("s3.exist", "false");
            attributes.put("s3.messagetype", "AmazonClientException");
            attributes.put("s3.message", ace.getMessage());
            if (ace instanceof AmazonS3Exception) {
                AmazonS3Exception s3e = (AmazonS3Exception) ace;
                attributes.put("s3.statuscode", String.valueOf(s3e.getStatusCode()));
                if (s3e.getStatusCode() == 404) {
                    if (bulletin) {
                        getLogger().error("Failed to retrieve S3 Object for {}; routing to failure", new Object[]{flowFile, ace});
                    }
                    else {
                        getLogger().info("S3 Object not found for {}; routing to failure", new Object[]{flowFile, ace});
                    }
                    if (penalize){
                        flowFile = session.penalize(flowFile);
                    }
            ...
            if (ExceptionUtils.indexOfType(ffae, AmazonClientException.class) != -1) {
                ...
                attributes.put("s3.key", key);
                attributes.put("s3.exist", "false");
                attributes.put("s3.messagetype", "FlowFileAccessException");
                attributes.put("s3.message", ffae.getMessage());
                flowFile = session.putAllAttributes(flowFile, attributes);
                flowFile = session.penalize(flowFile);
                session.transfer(flowFile, REL_FAILURE);
                return;
        ...
        if (!attributes.isEmpty()) {
            attributes.put("s3.exist", "true");
            attributes.put("s3.message", "S3 object found");
            flowFile = session.putAllAttributes(flowFile, attributes);
        }
